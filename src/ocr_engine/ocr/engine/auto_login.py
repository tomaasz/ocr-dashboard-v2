...
class AutoLogin:
    ...
    def _load_credentials(self) -> Optional[dict]:
        ...
        try:
            ...
            creds = profiles.get(self.profile_name)
            
            if not creds:
                logger.warning(f"[AutoLogin] No credentials for profile: {self.profile_name}")
                return None
    ...
    def perform_login(self, page: Page) -> bool:
        ...
        try:
            ...
            # Step 1: Click Sign in button if present
            sign_in_btn = page.locator("a:has-text('Sign in'), button:has-text('Sign in')").first
            if sign_in_btn.count() > 0 and sign_in_btn.is_visible(timeout=2000):
            ...
            if self._detect_sms_verification(page):
                logger.critical("🚨 [AutoLogin] SMS VERIFICATION REQUIRED - manual intervention needed!")
                self._log_sms_verification_event(page)
                return False
            
            # Step 5: Handle TOTP 2FA
            totp_input = page.locator("input[type='tel'], input[name='totpPin'], input[id='totpPin']").first
            if totp_input.is_visible(timeout=5000):
                ...
                # Click Next/Verify
                next_btn = page.locator("button:has-text('Next'), button:has-text('Dalej'), button:has-text('Verify')").first
                if next_btn.count() > 0:
                    next_btn.click()
                else:
                    page.keyboard.press("Enter")
            ...
            if "gemini.google.com" in page.url:
                ...
                logged_in = page.locator("img[aria-label*='Account'], button[aria-label*='Account']").first
                if logged_in.count() > 0:
                    logger.info("[AutoLogin] ✅ Login successful!")
                    return True
            
            # Check for error messages
            error = page.locator("div[aria-live='assertive'], span:has-text('Wrong password')").first
        ...
        try:
            ...
            self.db_manager.log_critical_event(
                ...
                meta={
                    ...
                    "email": self.credentials.get("email", "unknown") if self.credentials else "unknown",
                },
            )
