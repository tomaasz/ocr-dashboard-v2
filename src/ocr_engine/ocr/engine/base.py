...
class OcrEngine(Protocol):
    ...
    def run_job(
        self,
        job_dir: Path,
        stages: Optional[list[OcrStage]] = None,
    ) -> list[OcrResult]:
        """
...
class BaseOcrEngine(ABC):
    ...
    def load_job(self, job_dir: Path) -> dict:
        ...
        if not isinstance(image_size, dict):
            raise ValueError("job.json: missing or invalid 'ui.image_size' (expected object with w/h)")
        ...
        for idx, r in enumerate(rects):
            ...
            if x < 0 or y < 0 or w <= 0 or h <= 0:
                raise ValueError(f"job.json: ui.rects[{idx}] has invalid geometry (x,y>=0 and w,h>0)")

            if x + w > img_w or y + h > img_h:
                raise ValueError(f"job.json: ui.rects[{idx}] out of bounds for image_size w={img_w} h={img_h}")
    ...
    def run_job(self, job_dir: Path, stages: Optional[list[OcrStage]] = None) -> list[OcrResult]:
