...
from ocr_engine.ocr.engine.image_processor import preprocess_image_smart, clear_temp_images
...
class PrepWorker:
    ...
    def __init__(
        ...
        if os.path.commonpath([cwd, source_path_abs]) != cwd:
            raise ValueError(f"Security violation: Source path '{source_path_abs}' must be within project directory '{cwd}'")
            
        if os.path.commonpath([cwd, job_path_abs]) != cwd:
            raise ValueError(f"Security violation: Job path '{job_path_abs}' must be within project directory '{cwd}'")
    ...
    def _preprocess_file(self, source_file: Path) -> Optional[Path]:
    ...
    def run(self):
        """Main loop."""
        logger.info(f"🚀 Starting PrepWorker")
...
def main():
    ...
    if os.path.commonpath([cwd, source_path_abs]) != cwd:
        logger.error(f"❌ Security violation: Source path '{source_path_abs}' must be within project directory '{cwd}'")
        return
        
    if os.path.commonpath([cwd, job_path_abs]) != cwd:
        logger.error(f"❌ Security violation: Job path '{job_path_abs}' must be within project directory '{cwd}'")
        return
