...
class PromptManager:
    ...
    def render(self, prompt_id: str, meta: PromptMeta) -> str:
        ...
        text = "\n".join(template)
        return text.replace("__FILE_NAME__", meta.file_name).replace("__SOURCE_PATH__", meta.source_path)
    
    def setup_and_render(self, prompt_id: Optional[str], file_name: str, source_path: str) -> str:
