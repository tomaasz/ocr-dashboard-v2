...
class GeminiEngine:
    ...
    def __init__(
        ...
        try:
            self.workers_count = max(
                1,
                int(os.environ.get("OCR_WORKERS") or os.environ.get("OCR_DEFAULT_WORKERS") or "2"),
            )
        except Exception:
            self.workers_count = 2
        # Two worker tabs per window to overlap send/collect cycles.
        ...
        self.source_dir = Path(
            os.environ.get(
                "OCR_SOURCE_DIR",
                "/mnt/nas_genealogy/Sources/Nurskie dokumenty/1_43_0_4 Nurskie grodzkie wieczyste/01",
            )
        ).resolve()
        ...
        if not remote_for_profile:
            ...
            if proxies_file.exists():
                try:
                    ...
                    # Check specifics for this profile
                    # Check by active_profile_name (e.g., "0111hnswd")
                    self.proxy_config = proxies_map.get(self.active_profile_name)

                    # Fallback to default if not found
                    if not self.proxy_config:
                        self.proxy_config = proxies_map.get("default")
    ...
    def _update_live_previews(self) -> None:
        ...
        for w in self.workers:
            ...
            try:
                ...
                # We use jpeg for speed and smaller size
                # Namespace by profile to avoid collisions in shared batch dir
                safe_profile = sanitize_profile_name(self.active_profile_name)
                path = live_dir / f"{safe_profile}_w{w.wid}.jpg"
                w.page.screenshot(path=path, type="jpeg", quality=50)
    ...
    def _init_pages(self) -> None:
        ...
        for w in self.workers:
            try:
                w.page.goto("https://gemini.google.com/app", wait_until="domcontentloaded")
            except Exception as e:
    ...
    def _worker_try_collect(self, w: PageWorker) -> bool:
        ...
        try:
            ...
            if status != "response":
                raise RuntimeError(f"Collect failed: status={status}")
