...
MAX_DIMENSION = _env_int("OCR_PREPROC_MAX_DIMENSION", 3200)  # Max width/height (3200px ~= 275 DPI for A4, better for handwriting)
...
def _build_text_mask(img: np.ndarray) -> np.ndarray:
    ...
    open_k = cv2.getStructuringElement(cv2.MORPH_RECT, (TEXT_MASK_OPEN_KERNEL, TEXT_MASK_OPEN_KERNEL))
    close_k = cv2.getStructuringElement(cv2.MORPH_RECT, (TEXT_MASK_CLOSE_KERNEL, TEXT_MASK_CLOSE_KERNEL))
...
def _denoise(img: np.ndarray) -> np.ndarray:
    ...
    """
    return cv2.fastNlMeansDenoising(img, None, h=DENOISE_STRENGTH, templateWindowSize=7, searchWindowSize=21)
...
def _unsharp_mask(img: np.ndarray, amount: float = UNSHARP_AMOUNT, radius: int = UNSHARP_RADIUS) -> np.ndarray:
