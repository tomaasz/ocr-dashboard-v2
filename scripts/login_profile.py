...
def load_proxy_config(active_profile_name):
    """Load proxy config similar to gemini_engine.py"""
    try:
        ...
        proxy_config = proxies_map.get(active_profile_name)

        # Fallback to default if not found
        if not proxy_config:
            proxy_config = proxies_map.get("default")

        if proxy_config:
            logger.info(f"Using proxy for profile '{active_profile_name}'")
...
def main():
    ...
    proxy_config = load_proxy_config(profile_name)
