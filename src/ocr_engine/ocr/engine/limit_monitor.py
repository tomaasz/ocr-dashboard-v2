...
class LimitMonitor:
    ...
    def _get_pause_until(self, profile_name: str) -> Optional[datetime]:
    ...
    def _deep_check_profile(self, profile_name: str) -> Optional[datetime]:
        """
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                page.goto("https://gemini.google.com/app", wait_until="networkidle", timeout=30000)
                time.sleep(3)
                ...
                if re.search(PRO_LIMIT_TEXT_RE, body_text):
                    ...
                    logger.warning(f"⚠️ [{profile_name}] PRO LIMIT detected. Reset: {reset_time.strftime('%H:%M') if reset_time else 'unknown'}")
                    context.close()
                    return reset_time
                else:
            ...
            try:
                ...
                for profile, status in statuses.items():
                    ...
                    if pause_until and pause_until < datetime.now():
                        ...
                        statuses[profile] = {"status": "AVAILABLE", "until": None, "remaining_min": 0}
                    ...
                    if now - last_deep > self.deep_check_interval:
                        ...
                        if reset_time:
                            ...
                            statuses[profile] = {
                                ...
                                "remaining_min": int((reset_time - datetime.now()).total_seconds() / 60),
                            }
                        else:
                            self._clear_pause(profile)
                            statuses[profile] = {"status": "AVAILABLE", "until": None, "remaining_min": 0}
...
def main():
    ...
    parser.add_argument(
        "--cache-dir",
        default=os.path.expanduser("~/.cache/ocr-dashboard-v2"),
