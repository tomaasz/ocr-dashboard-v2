...
PRO_LIMIT_TEXT_RE = re.compile(
    r"(Osiągnięto limit modelu Pro|Reached Pro model limit|Limit zostanie zresetowany|Limit resets)",
    re.IGNORECASE,
)
...
class ProLimitHandler:
    ...
    def __init__(
        ...
        pro_only: bool = True,
        release_locks_callback: Optional[Callable[[], None]] = None,
        page_reload_callback: Optional[Callable[[], None]] = None,
    ):
        ...
        try:
            self.fallback_pause_min = int(os.environ.get("OCR_PRO_FALLBACK_PAUSE_MIN", "60").strip())
        except Exception:
    ...
    def extract_reset_datetime_from_text(self, text: str) -> Optional[datetime]:
        """
    ...
    def pause_until(self, until: datetime, reason: str = "pro_limit") -> None:
        ...
        logger.warning(f"🛑 [PRO-ONLY] PAUSE until {reset_time_str} ({int(wait_seconds/60)} min). Releasing locks...")
    ...
    def trigger_pause_from_text(self, text: str, context: str = "") -> None:
        ...
        logger.warning(f"⚠️ [PRO-ONLY] PRO LIMIT detected{ctx}. Reset ~ {until.strftime('%Y-%m-%d %H:%M')}.")
